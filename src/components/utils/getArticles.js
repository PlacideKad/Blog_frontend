...
export const getArticles=async(setFunction, backendURL, isPublished=true,limit=6,page=1,setTotalPages=undefined,search_input=null,exact=false,sort_by='createdAt',order=1)=>{
  try{
    const res=await fetch(`${backendURL}/${isPublished?'articles':'admin/stashes'}/?${limit?`limit=${limit}`:' '}&${page&&`page=${page}`}&${search_input?.trim()?`search=${search_input.trim()}&exact=${exact?1:0}`:'/'}${(sort_by && order)?`&order=${order}&sort_by=${sort_by}`:" "}`);
